...
import (
	sap_api_caller "sap-api-integrations-competitor-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-competitor-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-competitor-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	competitorID := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetCompetitor(competitorID, accepter)
	return nil
}

func extractData(data map[string]interface{}) (competitorID string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	competitorID = sdc.CompetitorCollection.CompetitorID
	return
}
